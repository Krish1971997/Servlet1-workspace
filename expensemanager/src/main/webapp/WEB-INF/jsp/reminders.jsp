...
.reminder-hero::before {
	...
	width: 280px;
	height: 280px;
	background: radial-gradient(circle, rgba(245, 158, 11, .2), transparent
		70%);
	top: -80px;
	right: -40px;
	border-radius: 50%
...
.reminder-card.highlight {
	...
	animation: highlight-flash .6s ease
}

@
keyframes highlight-flash { 0%{
...
.toast-accent.bill_due {
	background: #f59e0b
}

.toast-accent.due_today {
	background: #ef4444;
	animation: flash-bar 1s ease-in-out infinite
}

.toast-accent.overdue {
	background: #ef4444
}

.toast-accent.auto_expense {
	background: #10b981
}
...
@
keyframes toastIn {
	from {transform: translateX(110%) scale(.9);
	opacity: 0
}
...
<body>
	...
	<nav class="sidebar">
		...
		<ul class="nav-links">
			...
			<li><a href="${pageContext.request.contextPath}/reminders/list"
				class="active"><i class="fas fa-bell"></i><span>Reminders</span></a></li>
	...
	<main class="main-content">
		<div class="topbar">
			...
			<div class="topbar-actions" style="gap: 12px">
				...
				<span class="sse-status sse-offline" id="sseStatus"> <span
					class="sse-dot"></span> <span id="sseLabel">Connecting…</span>
				</span>
				<!-- Bell with badge -->
				<div class="bell-wrap" id="bellWrap" onclick="openPanel()"
					title="Notifications">
					<i class="fas fa-bell"
						style="font-size: 1.1rem; color: var(--text-secondary)"
						id="bellIcon"></i> <span class="bell-badge" id="bellBadge"
						style="display: none">0</span>
				</div>
				<a href="${pageContext.request.contextPath}/reminders/add"
					class="btn btn-primary btn-sm"> <i class="fas fa-plus"></i> Add
					Reminder
				</a>
			</div>
		</div>
		...
		<div class="page-body">
			...
			<div class="reminder-hero">
				<div class="hero-text">
					<h2>
						<i class="fas fa-bolt" style="color: #f59e0b"></i> &nbsp;Real-Time
						Bill Alerts
					</h2>
					<p>
						SSE-powered push notifications — EB bill, recharge, credit card,
						EMI.<br> Get alerted the moment a bill is due. No page
						refresh needed.
					</p>
				</div>
				<a href="${pageContext.request.contextPath}/reminders/add"
					class="btn btn-primary"> <i class="fas fa-plus"></i> Add Bill
				</a>
			</div>
			...
			<c:forEach var="r" items="${reminders}">
				...
				<c:if test="${r.dueToday}">
					<c:set var="dueTodayCount" value="${dueTodayCount+1}" />
				</c:if>
			...
			<div class="stats-row">
				<div class="stat-card">
					...
					<div class="stat-info">
						<label>Overdue</label><span>${overdueCount}</span>
					</div>
				</div>
				<div class="stat-card">
					...
					<div class="stat-info">
						<label>Due Today</label><span>${dueTodayCount}</span>
					</div>
				</div>
				<div class="stat-card">
					...
					<div class="stat-info">
						<label>Due Soon</label><span>${dueSoonCount}</span>
					</div>
				</div>
				<div class="stat-card">
					...
					<div class="stat-info">
						<label>Live Tabs</label><span id="sseCount">${sseCount}</span>
					</div>
				</div>
			</div>
			...
			<c:choose>
				<c:when test="${empty reminders}">
					<div class="empty-state">
						...
						<p>
							No reminders yet. <a
								href="${pageContext.request.contextPath}/reminders/add"
								style="color: var(--accent)">Add your first bill →</a>
						</p>
					</div>
				</c:when>
				<c:otherwise>
					<div class="reminders-grid" id="remindersGrid">
						<c:forEach var="r" items="${reminders}">
							<div class="reminder-card ${!r.active ? 'inactive' : ''}"
								id="card-${r.id}" data-id="${r.id}" data-title="${r.title}">
								<div class="card-header">
									<div style="display: flex; align-items: center; gap: 10px">
										<div class="card-icon"
											style="background: rgba(99, 102, 241, .12); color: #818cf8">
											<c:choose>
												<c:when test="${r.category == 'Utilities'}">
													<i class="fas fa-bolt"></i>
												</c:when>
												<c:when test="${r.category == 'Insurance'}">
													<i class="fas fa-shield-halved"></i>
												</c:when>
												<c:when test="${r.category == 'Health'}">
													<i class="fas fa-heart-pulse"></i>
												</c:when>
												<c:when test="${r.category == 'Entertainment'}">
													<i class="fas fa-tv"></i>
												</c:when>
												<c:when test="${r.category == 'Transport'}">
													<i class="fas fa-car"></i>
												</c:when>
												<c:when test="${r.category == 'Subscriptions'}">
													<i class="fas fa-rss"></i>
												</c:when>
												<c:when test="${r.category == 'Loans'}">
													<i class="fas fa-landmark"></i>
												</c:when>
												<c:when test="${r.category == 'Education'}">
									...
									<div class="card-actions">
										<a
											href="${pageContext.request.contextPath}/reminders/edit?id=${r.id}"
											class="act-btn ab-edit" title="Edit"><i
											class="fas fa-pen"></i></a> <a
											href="${pageContext.request.contextPath}/reminders/paid?id=${r.id}"
											class="act-btn ab-paid" title="Mark Paid"
											onclick="return confirm('Mark \'${r.title}\' as paid and add expense?')"><i
											class="fas fa-check"></i></a> <a
											href="${pageContext.request.contextPath}/reminders/toggle?id=${r.id}"
											class="act-btn ab-toggle"
											title="${r.active ? 'Pause' : 'Activate'}"> <i
											class="fas ${r.active ? 'fa-pause' : 'fa-play'}"></i></a>
										<button class="act-btn ab-del" title="Delete"
								...
								<div class="card-amount">
									₹
									<fmt:formatNumber value="${r.amount}" pattern="#,##0.00" />
								</div>

								<div class="card-meta">
									<span class="freq-badge"><i class="fas fa-rotate"
										style="font-size: .6rem"></i> ${r.frequency}</span> <span
										class="cat-badge">${r.category}</span>
									<c:if test="${r.autoAddExpense}">
										<span class="auto-badge"><i class="fas fa-bolt"
											style="font-size: .6rem"></i> Auto</span>
									</c:if>
								</div>

								<div
									style="display: flex; align-items: center; justify-content: space-between">
									<span class="status-chip ${r.statusClass}"> <i
										class="fas ${r.overdue ? 'fa-circle-exclamation' : r.dueToday ? 'fa-bell' : 'fa-calendar-day'}"
										style="font-size: .6rem"></i> ${r.statusLabel}
									</span> <span style="font-size: .75rem; color: var(--text-muted)">
								...
								<c:if test="${!r.overdue && r.daysUntilDue <= 30}">
									<div class="due-bar-wrap">
										<div class="due-bar-fill"
											style="width:${100 - (r.daysUntilDue / 30.0 * 100)}%;background:${r.dueToday ? '#f59e0b' : r.dueSoon ? '#fbbf24' : '#10b981'}"></div>
									</div>
								</c:if>
							</div>
						</c:forEach>
			...
			<c:if test="${not empty recentAlerts}">
				<div class="section-card">
					<div class="section-head">
						<h3>
							<i class="fas fa-history" style="color: #f59e0b"></i> Recent
							Notifications
						</h3>
						<span style="font-size: .78rem; color: var(--text-muted)">${recentAlerts.size()}
							records</span>
					</div>
						...
						<c:forEach var="n" items="${recentAlerts}">
							<div class="notif-item">
								...
								<div class="notif-text">
									...
									&nbsp;·&nbsp;Due
									<fmt:formatDate value="${n.nextDueDate}" pattern="dd MMM" />
									&nbsp;·&nbsp;
									<c:choose>
								...
								<div class="notif-time">
									<fmt:formatDate value="${n.sentAt}" pattern="dd MMM HH:mm" />
								</div>
							</div>
						</c:forEach>
					</div>
	...
	<div class="notif-panel" id="notifPanel">
		<div class="np-header">
			...
			<button class="np-close" onclick="closePanel()">
		...
		<div class="np-body" id="npBody">
			<div class="np-empty" id="npEmpty">
				<i class="fas fa-bell-slash"
					style="font-size: 2rem; opacity: .25; display: block; margin-bottom: 12px"></i>No
				notifications yet.<br>Bills will appear here in real-time.
			</div>
		</div>
		<div class="np-footer">
	...
	<div id="panelOverlay" onclick="closePanel()"
		style="display: none; position: fixed; inset: 0; z-index: 1999; background: rgba(0, 0, 0, .4)"></div>
	...
	<div class="modal-overlay" id="deleteModal">
		<div class="modal-box">
			...
			<p>
				Permanently delete <strong id="delTitle"
