...
public class PostDAO {
    ...
    public int createPost(int authorId, Integer groupId, String content) {
        ...
        try (Connection conn = DBUtil.getConnection();
            ...
            if (groupId != null) ps.setInt(2, groupId); else ps.setNull(2, Types.INTEGER);
    ...
    /**
     * Returns wall posts.
     * - If caller is admin: all non-hidden + own hidden.
     * - If caller is regular user: non-hidden + posts shared with caller.
     */
    public List<Post> getWallPosts(int callerId, boolean isAdmin) {
        ...
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            if (!isAdmin) ps.setInt(1, callerId);
            try (ResultSet rs = ps.executeQuery()) {
    ...
    public List<Post> getGroupPosts(int groupId, int callerId, boolean isAdmin) {
        ...
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, groupId);
            if (!isAdmin) ps.setInt(2, callerId);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) list.add(mapRow(rs));
            }
        } catch (SQLException e) { e.printStackTrace(); }
    ...
    public boolean sharePost(int postId, List<Integer> userIds) {
        ...
        try (Connection conn = DBUtil.getConnection()) {
            ...
            try (PreparedStatement del = conn.prepareStatement(delSql)) {
                del.setInt(1, postId); del.executeUpdate();
            }
            try (PreparedStatement ins = conn.prepareStatement(insSql)) {
                for (int uid : userIds) {
                    ins.setInt(1, postId); ins.setInt(2, uid); ins.addBatch();
                }
                ins.executeBatch();
            }
            conn.commit();
